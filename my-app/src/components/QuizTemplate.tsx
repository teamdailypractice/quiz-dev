...
const QuizTemplate: React.FC<QuizProps> = QuizProps => {
  ...
  const selectOption = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    ...
    // Get answer buttons
    if (answerButtonsRef !== null &&  answerButtonsRef.current !== null) {
      const answerBtnsElements:HTMLElement = answerButtonsRef.current;
      const answerBtns = Array.from(answerBtnsElements.getElementsByClassName("answers-btns"));
